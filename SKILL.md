...
# 自定义请求头（防盗链 Referer / 鉴权 Authorization / Cookie）
uv run scripts/web_fetch.py "https://d1.weather.com.cn/sk_2d/101020100.html" \
...
- **测试套件**：`bash tests/run_tests.sh` —— 50 条确定性断言，全部打本地 mock 服务器，无需联网。覆盖后端、输出格式、参数边界、GBK、gzip/deflate、二进制拒绝、HTTP 错误、重定向、超时、重试恢复、截断、search→fetch 集成、并发。
